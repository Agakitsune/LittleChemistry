...
var link_toward: Node3D
var solid_link: bool = false
var notifier: Node3D = null

func _process(delta):
	if link_toward:
		var distance = global_position.distance_to(link_toward.global_position)
		var target_vector = global_position.direction_to(link_toward.global_position)
		for link in get_tree().get_nodes_in_group("Links"):
			link.scale.y = distance / 2
			link.position.y = distance / 2
		basis = Basis.looking_at(target_vector)
		var tmp = basis.y
		basis.y = -basis.z
		basis.z = tmp

func _no_link(object):
	notifier = object
	for link in get_tree().get_nodes_in_group("Links"):
		link.visible = false

func _on_area_3d_area_entered(area):
	if area.is_in_group("AtomCollide"):
		var link = link_scene.instantiate()
		add_child(link)
		link.add_to_group("Links")
		link.global_position = $Atom.global_position
		link_toward = area
		link.get_surface_override_material(0).albedo_color = Color(1.0,1.0,1.0,0.2)
		if not notifier:
			area.on_collide.emit(self)
		#$Atom.get_surface_override_material(0).albedo_color = Color(0.0,1.0,0.0)
...
func _on_area_3d_area_exited(area):
	if area.is_in_group("AtomCollide"):
		...
		link_toward = null
		notifier = null
		#$Atom.get_surface_override_material(0).albedo_color = Color(1.0,0.0,0.0)

func _on_dropped(pickable):
	for link in get_tree().get_nodes_in_group("Links"):
		link.get_surface_override_material(0).albedo_color = Color(1.0,1.0,1.0,1.0)
